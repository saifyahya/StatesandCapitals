...
public class StatesAndCapitals
{
    ...
    public void testStatesAndCapitals() throws IOException
    {
        ...
        List<StateInfo> lastFiveStates = states.stream().skip(statesSize-5).collect(toList());
        ...
        StateInfo cardinalState = states.stream().filter(x->x.getStateBird().equals("cardinal")).findAny().orElseThrow();
        ...
        Boolean doAllStatesHaveAnAnthem = states.stream().allMatch(state -> !state.getStateAnthem().isEmpty());
        ...
        List<String> allDistinctStateBirdsMinusMockingbirds = states.stream().map(StateInfo::getStateBird).filter(bird-> !bird.toLowerCase().contains("mockingbird")).distinct().collect(toList());
